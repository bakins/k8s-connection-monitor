// Package connection provides operations to get connections
package connection
...
import (
	...
	monitor "github.com/bakins/k8s-connection-monitor/pkg/monitor"
...
// This only works on Linux, but contains no OS specific code(ie, no CGO)
func GetConnections(root string, pid int) ([]monitor.Connection, error) {
	...
	for _, k := range connectionKindTypes {
		filename := filepath.Join(root, strconv.Itoa(pid), "net", k.filename)
